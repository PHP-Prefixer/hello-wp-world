...
use Closure;
use PPP\Illuminate\Contracts\Mail\Factory;
use PPP\Illuminate\Contracts\Mail\Mailable;
use PPP\Illuminate\Contracts\Mail\Mailer;
use PPP\Illuminate\Contracts\Mail\MailQueue;
use PPP\Illuminate\Contracts\Queue\ShouldQueue;
...
class MailFake implements Factory, Mailer, MailQueue
{
    ...
    {
        $mailableNames = PPP_collect($this->mailables)->map(function ($mailable) {
    ...
    {
        $mailableNames = PPP_collect($this->queuedMailables)->map(function ($mailable) {
            return get_class($mailable);
        })->join(', ');
    ...
    {
        return PPP_collect($this->mailables)->filter(function ($mailable) use ($type) {
    ...
    {
        return PPP_collect($this->queuedMailables)->filter(function ($mailable) use ($type) {
            return $mailable instanceof $type;
        });
    }

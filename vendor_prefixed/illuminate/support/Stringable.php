...
use Closure;
use PPP\Illuminate\Support\Traits\Conditionable;
use PPP\Illuminate\Support\Traits\Macroable;
use PPP\Illuminate\Support\Traits\Tappable;
...
class Stringable implements JsonSerializable
{
    ...
    {
        return new static(PPP_class_basename($this->value));
    }
    ...
    {
        return PPP_collect(explode($delimiter, $this->value, $limit));
    }
    ...
    public function split($pattern, $limit = -1, $flags = 0)
    {
        if (filter_var($pattern, FILTER_VALIDATE_INT) !== false) {
            return PPP_collect(mb_str_split($this->value, $pattern));
        }

        $segments = preg_split($pattern, $this->value, $limit, $flags);

        return ! empty($segments) ? PPP_collect($segments) : PPP_collect();
    }

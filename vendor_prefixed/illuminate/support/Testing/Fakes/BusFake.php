...
use PPP\Illuminate\Contracts\Bus\QueueingDispatcher;
use PPP\Illuminate\Support\Arr;
use PPP\Illuminate\Support\Collection;
use PPP\Illuminate\Support\Traits\ReflectsClosures;
...
class BusFake implements QueueingDispatcher
{
    ...
    public function assertChained(array $expectedChain)
    {
        ...
        PHPUnit::assertTrue(
            PPP_collect($expectedChain)->isNotEmpty(),
    ...
    {
        return PPP_tap(clone $job, function ($job) {
    ...
    {
        $chain = PPP_collect($expectedChain)->map(function ($job) {
    ...
    protected function assertDispatchedWithChainOfClasses($command, $expectedChain, $callback)
    {
        $matching = $this->dispatched($command, $callback)->map->chained->map(function ($chain) {
            return PPP_collect($chain)->map(function ($job) {
    ...
    {
        return ! PPP_collect($chain)->contains(function ($job) {
    ...
    public function dispatched($command, $callback = null)
    {
        ...
        return PPP_collect($this->commands[$command])->filter(function ($command) use ($callback) {
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return PPP_collect($this->commandsSync[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return PPP_collect($this->commandsAfterResponse[$command])->filter(function ($command) use ($callback) {
            return $callback($command);
        });
    }
    ...
    public function batched(callable $callback)
    {
        ...
        return PPP_collect($this->batches)->filter(function ($batch) use ($callback) {
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return PPP_collect($this->jobsToFake)
